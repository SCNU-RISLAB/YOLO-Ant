...
import contextlib
import glob         # python自带的一个文件操作相关模块 查找符合自己目的的文件（如模糊匹配）
import hashlib      # 哈希模块 提供了多种安全方便的hash方法
import json         # json文件操作模块
import math         # 数学公式模块
import os           # 与操作系统进行交互的模块 包含文件路径和解析
import random       # 生成随即模块
import shutil       # 文件夹压缩包处理模块
import time         # 时间模块 更底层
from itertools import repeat                         # 复制模块
from multiprocessing.pool import Pool, ThreadPool    # 多线程模块 线程池
from pathlib import Path                             # Path将str转为Path对象 使字符串路径易于操作模块
from threading import Thread                         # 多线程操作模块
from urllib.parse import urlparse

import numpy as np                                  # numpy矩阵模块
...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(self,
        ...
        self.labels = list(labels)     # 所有图片的gt框信息
        self.shapes = np.array(shapes) # 所有图片的shape
