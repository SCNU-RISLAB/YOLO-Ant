...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    #init_seeds(opt.seed + 1024, deterministic=True)
    ...
    if pretrained:
        ...
        with torch_distributed_zero_first(LOCAL_RANK):
            # 这里下载是去google云盘下载，建议自行下载后放在weights中
    ...
    # Model attributes
    nl = de_parallel(model).model[-1].nl  # number of detection layers (to scale hyps) 代表有多少个检测层（头）
...
def parse_opt(known=False):
    ...
    parser.add_argument('--cfg', type=str, default='models/YOLO-Ant.yaml', help='model.yaml path')
    parser.add_argument('--data', type=str, default=ROOT / 'data/Antenna.yaml', help='dataset.yaml path')
    ...
    parser.add_argument('--project', default=ROOT / 'runs/train/vis', help='save to project/name')
    parser.add_argument('--name', default='our_vispy', help='save to project/name')
    ...
    parser.add_argument('--acmix', action='store_true', help='cosine LR scheduler', default=True)
    # Logger arguments
...
def main(opt, callbacks=Callbacks()):
    ...
    #init_epochs = opt.epochs #训练完后继续训练
    if opt.resume and not check_comet_resume(opt) and not opt.evolve:
        ...
        #opt.cfg, opt.weights, opt.resume, opt.epochs = '', str(last), True, init_epochs  #训练完后继续训练
        if is_url(opt_data):
